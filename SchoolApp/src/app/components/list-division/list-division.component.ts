...
export class ListDivisionComponent implements OnInit {
  ...
  constructor(private router: Router, private broadcaster: Broadcaster, private _division: DivisionService) {
  }
  ...
  delete(row: any, go: any) {
    if (go) {
       this._division.deleteDivision(this.deleteCache.divisionId).subscribe((res) => {
        }, (resError) => {
        });
     } else {
