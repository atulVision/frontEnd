...
export class AlbumService {
  ...
  updateAlbum(albumId: any, album: Album) {
    return this._http.put(Constants.URL.host_url + Constants.URL.bookURL + '/' + albumId, album)
      .map((response: Response) => {
        return response.json();
      })
      .catch(this._errorHandler);
  ...
  deleteAlbum(albumId: any) {
    return this._http.delete(Constants.URL.host_url + Constants.URL.bookURL + '/' + albumId)
      .map((response: Response) => {
        return response.json();
      })
      .catch(this._errorHandler);
